...
- 撤回错误的大面积连续遮罩方案，避免透明面跨墙段覆盖、污染背景墙和视野切线。
- 摄像机挡墙只处理摄像机到人物射线真正命中的同一条前景墙平面；背景墙、未命中的墙不再参与透明。
- 前景墙透明改为世界坐标 shader 蒙版：中心透明、边缘按距离平滑恢复，所有同平面墙板共享同一个渐变中心，减少块与块 alpha 不一致造成的接缝。
- 灰色记忆区域加深一档，但仍是静态记忆，不受实时灯光影响。
- 验证：`validate_all.bat` 通过。
