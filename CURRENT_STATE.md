...
- 已修正墙角/门边盲区墙面误亮：墙面实时可见不再只看墙体采样点本身，还要求墙前方探测点在物理视野内，并且多个墙面采样点同时可见后才切到实时贴图，减少盲区墙面被灯光点亮。
- 已继续柔化盲区记忆：实时亮面和已见记忆分离，弱可见或刚被遮挡的墙/地板会先过渡到稍暗灰色记忆；只有未被记录为见过的结构才保持全黑。记忆灰色亮度上调，避免看起来像黑色马赛克。
- 已修正“灯板被遮挡就不照亮周围”的逻辑：真实 `Light3D` 现在会检测自己是否能照到当前玩家视野里的地面采样点，只要照得到可见区域就保持工作；灯板 mesh 仍然按玩家视线遮挡单独显示。
- 已扩大镜头挡墙透现范围：挡墙透明不再只处理射线命中的单块墙板，而是以命中点为中心让周围墙板按距离渐变透明；中心更透明，边缘逐步恢复不透明，切线透明度和显示范围也降低，减少硬边/马赛克感。
- 已修正灯光可达和切线马赛克：真实灯光改为检测灯到可见地面区域的水平物理路径，不再因为灯板本体丢失玩家视线就关灯；地板记忆不再整块压暗，只保留轻微静态色差，真实视野边界改用更宽更淡的连续遮罩线；镜头挡墙淡出改为附近墙体统一透明，并关闭逐块竖向切线，减少墙板拼缝。
