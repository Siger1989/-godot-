...
enum State { WANDER, IDLE_LOOK, FLEE, CHASE, INVESTIGATE, ATTACK, DEAD }
...
@export var wander_max_time := 4.5
@export var idle_min_time := 1.0
@export var idle_max_time := 2.4
...
@export var monster_footstep_volume_db := -13.0
...
func _update_nightmare_state(delta: float) -> void:
	...
	if audible_target != null:
		...
		_last_heard_position = audible_target.global_position
		_has_last_heard_position = true
		_last_seen_position = _last_heard_position
		_has_last_seen_position = true
		_state_timer = nightmare_hearing_memory_time
...
func _update_investigate(delta: float) -> void:
	...
	if to_target.length() <= investigate_reach_distance:
		_choose_wander()
		return
	_move_horizontal(to_target.normalized(), wander_speed * 1.25, delta)
...
func _perform_attack() -> void:
	if (monster_role != "red" and monster_role != "nightmare") or _chase_target == null or _state == State.DEAD:
		return
...
func _find_room_center_for_area(area_id: String) -> Vector3:
	...
	for room in _rooms_root.get_children():
		...
		if room_node == null:
			continue
		var area_variant: Variant = room.get("area_id")
		if area_variant == null or String(area_variant) != area_id:
			continue
...
func _can_sense_target(target: Node3D, target_height: float, allow_panic: bool) -> bool:
	...
	if allow_panic and distance <= panic_distance and _has_clear_line_to_node(target, target_height):
		return true
	...
	if forward.dot(direction) < minimum_dot:
		return false
	return _has_clear_line_to_node(target, target_height)
...
func _has_clear_line_to_player() -> bool:
	if _player == null or not is_inside_tree():
		return false
	var ray_from := global_position + Vector3.UP * eye_height
	var ray_to := _player.global_position + Vector3.UP * player_target_height
	var query := PhysicsRayQueryParameters3D.create(ray_from, ray_to)
	var exclude: Array[RID] = []
	exclude.append(get_rid())
	query.exclude = exclude
