...
@export var mobile_joystick_radius := 92.0
@export var mobile_joystick_margin := Vector2(160.0, 144.0)
...
func _is_sprinting() -> bool:
	return Input.is_action_pressed("sprint") or _mobile_sprint_pressed
...
func _create_mobile_controls() -> void:
	...
	_mobile_sprint_button.custom_minimum_size = Vector2(118.0, 70.0)
	_mobile_sprint_button.add_theme_font_size_override("font_size", 22)
...
func _position_mobile_controls() -> void:
	...
	if _mobile_sprint_button != null:
		var button_size := Vector2(118.0, 70.0)
		_mobile_sprint_button.size = button_size
		_mobile_sprint_button.position = Vector2(viewport_size.x - button_size.x - 76.0, viewport_size.y - button_size.y - 154.0)
	_update_mobile_stick_knob()
...
func _update_interaction_button() -> void:
	if _interaction_button == null:
		return
	if _interaction_locked or not visible:
		_interaction_button.visible = false
		return
...
func _create_player_audio() -> void:
	...
	_footstep_player.volume_db = -17.0
...
func _update_player_audio(delta: float, input_vector: Vector2, sprinting: bool) -> void:
	...
	if moving and not _footstep_streams.is_empty():
